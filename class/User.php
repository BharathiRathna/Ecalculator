...
class User
{
    ...
    {
        if($this->role && $this->email && $this->password && $_SESSION["user_id"])
        {
    ...
    public function update()
    {
        if($this->role && $this->email && $_SESSION["userid"])
        {
            ...
            $stmt = $this->conn->prepare("UPDATE ".$this->userTable." SET first_name = ?, last_name = ?,email = ?,role = ? $updatePass WHERE id = ? ");
